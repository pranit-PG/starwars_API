...
def get_db_conn_toml():
 ...
 """
    ...
    with open("settings/secrets.toml", "r") as fp:
        config = toml.load(fp)
        dbconfig = config.get("mysqldb")   # as we only want db related data to connect db,we dont want "swapi" here
        conn_ = pymysql.connect(**dbconfig) # ** is used to pass this dbconfig data as dictionary(**kwargs)to connection
        return conn_
