...
import json              # for extracting data from webAPI of Postman
import requests
...
def generate_15_random_numbers():
    ...
    result = []
    while i <= 15:
        result.append(random.randint(1, 83))     #creates random number
        i += 1
    return result
...
def get_url(resource_id):
    ...
    relative_url = "/api/people/{}"   # curly bracket is used for inserting value into that url(people1,people2 etc)
    absolute_url = home_url + relative_url.format(resource_id)   #used .format to insert value
    return absolute_url
